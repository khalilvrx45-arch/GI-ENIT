...
export default function UserHeader({ profile }: { profile: Profile }) {
  ...
  return (
    <header className="sticky top-[76px] z-20 flex h-16 items-center justify-between border-b border-[#2a2c2c] bg-[#0d0e0e]/90 px-4 backdrop-blur-md lg:px-8">
      ...
      <div className="flex items-center gap-3">
        <div className="bg-[#141515] border border-[#2a2c2c] flex items-center gap-2 rounded-xl px-3 py-1.5">
          <span className="text-[10px] text-[#666] uppercase tracking-wider font-bold hidden sm:block">Points</span>
          <motion.span 
        ...
        <button
          ...
          className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-[#2a2c2c] bg-[#141515] text-[#888] transition-colors hover:border-red-500/40 hover:bg-red-500/10 hover:text-red-400 disabled:opacity-50"
        >
          {loggingOut ? (
